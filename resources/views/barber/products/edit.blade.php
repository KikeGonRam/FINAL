...
<body class="bg-gray-100 font-sans">
    ...
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('barber.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" class="form-control w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ $product->name }}" required>
            </div>

            <!-- Descripción del producto -->
            <div class="mb-4">
                <label for="description" class="block text-gray-700 text-lg font-medium mb-2">Descripción</label>
                <textarea name="description" class="form-control w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>{{ $product->description }}</textarea>
            </div>
    ...
    <script>
        ...
        function previewImage(event) {
            ...
            reader.onload = function() {
                ...
                imagePreviewContainer.style.display = 'flex'; // Muestra el contenedor de la vista previa
            };
            if (file) {
