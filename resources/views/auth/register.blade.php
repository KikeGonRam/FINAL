...
<html lang="es">
  <head>
    ...
    <meta name="description" content="Página de registro para nuevos usuarios" />
  ...
  <body class="bg-gradient-to-br from-gray-50 to-gray-100 font-dmSans min-h-screen flex justify-center items-center p-4">
    <article class="bg-white p-8 lg:p-12 rounded-2xl shadow-xl grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-4xl w-full relative overflow-hidden animate-fade-in">
      ...
      <form method="POST" action="{{ route('register') }}" class="relative z-10 space-y-6 animate-slide-up" novalidate>
        @csrf
        ...
        <div class="form-group relative">
          <i class="bx bxs-user absolute left-0 top-1/2 -translate-y-1/2 transition-all duration-300 text-gray-500"></i>
          <input 
            type="text" 
            id="name"
            name="name" 
    ...
    <script>
      ...
      form.addEventListener('submit', async (e) => {
        ...
        if (password.value !== confirmation.value) {
          isValid = false;
          password.parentElement.classList.add('error-shake');
          confirmation.parentElement.classList.add('error-shake');
          setTimeout(() => {
            password.parentElement.classList.remove('error-shake');
            confirmation.parentElement.classList.remove('error-shake');
          }, 820);
        }
        
        if (isValid) {
          // Animación de transición
          blackBlock.style.transform = 'rotate(-57deg) translateX(-100%)';
