...
<body class="bg-dark-400 text-white flex min-h-screen">
    ...
    <div class="flex-1 ml-64 p-8">
        ...
        <div class="flex justify-between items-center mb-4">
                <h1 class="text-2xl font-semibold text-beige mb-4">Gestionar Promociones</h1>
                <a href="{{ route('admin.promotions.create') }}" class="btn btn-primary py-2 px-4 bg-beige text-dark-100 rounded-lg hover:bg-dark-200 transition-colors">Crear Promoción</a>
        </div>
        ...
        <div class="overflow-x-auto bg-dark-200 rounded-lg shadow-lg">
            <table class="table-auto w-full text-sm text-left text-gray-400">
                <thead>
                    <tr>
                        ...
                        <th class="py-3 px-4 text-left text-beige">Descuento</th>
                        <th class="py-3 px-4 text-left text-beige">Tipo</th>
                        <th class="py-3 px-4 text-left text-beige">Fecha de Inicio</th>
                        <th class="py-3 px-4 text-left text-beige">Fecha de Fin</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="py-3 px-4 text-white">{{ $promotion->name }}</td>
                        <td class="py-3 px-4 text-white">{{ $promotion->description }}</td>
                        <td class="py-3 px-4 text-white">{{ $promotion->discount }}%</td>
                        <td class="py-3 px-4 text-white">{{ ucfirst($promotion->type) }}</td>
                        <td class="py-3 px-4 text-white">{{ $promotion->start_date }}</td>
                        <td class="py-3 px-4 text-white">{{ $promotion->end_date }}</td>
                        <td class="py-3 px-4 text-center">
                            <a href="{{ route('admin.promotions.edit', $promotion) }}" class="btn btn-warning bg-yellow-500 hover:bg-yellow-600 text-white py-2 px-4 rounded-lg">Editar</a>
                            <form action="{{ route('admin.promotions.destroy', $promotion) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de eliminar este producto?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger bg-red-600 hover:text-red-700 text-white py-2 px-4 rounded-lg">Eliminar</button>
                            </form>
                        </td>
                    </tr>
                    @endforeach
