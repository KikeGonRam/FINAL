...
<body class="bg-dark-400 text-white font-poppins min-h-screen flex">
    ...
    <main class="flex-1 ml-64 p-8">
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <!-- Header -->
            <header class="mb-8">
                <h1 class="text-3xl font-semibold text-white">Editar Servicio</h1>
                    <p class="text-gray-400 mt-2">Modifica los datos</p>
            </header>
            @if(session('success'))
            ...
            <div class="bg-dark-300 rounded-xl shadow-xl p-6">
                <form action="{{ route('admin.services.update', $service->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        <label for="name" class="block text-sm font-medium text-beige">
                            Disponibilidad para Fechas Especiales:</label>
                        <input type="checkbox" 
                                name="special_dates" 
                                value="1" {{ $service->special_dates ? 'checked' : '' }}>
                    </div>
                    <div class="space-y-2">
                        <label for="name" class="block text-sm font-medium text-beige">
                            Paquetes Especiales:</label>
                        <textarea name="special_packages" 
                                    class="w-full px-4 py-2 bg-dark-200 border border-dark-100 rounded-lg focus:outline-none focus:border-beige text-white placeholder-gray-500 form-input"
                                    >{{ $service->special_packages }}
                        </textarea>
                    </div>
